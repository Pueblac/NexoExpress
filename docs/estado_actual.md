...
**Cursor:** G0 → F1 → F2 (Gemini APROBADO CON CAMBIOS + anti-bluff válida) → **G1 CONCEDIDO (22-07):** diseño adoptado con los 6 cambios; **ventana POR MODO = 5 s presencial / 10 s virtual** (OBS-2 resuelta a favor de la reconciliación del Arquitecto); cambio 5 marcado ya-implementado. **Diseño CONGELADO** (ver `respuesta_diseno_ronda1.md` + parámetros abajo). **Siguiente: F3 (5 specs) → F4 rotación asistida (Sonnet) → F5.**
